            ...
            {presentAttendees.length > 0 ? (
              <ul className="space-y-1">
                {presentAttendees.map(attendee => (
                  <li key={attendee.id} className="text-sm p-2 bg-secondary/30 rounded-md" data-ai-hint="person name">{attendee.name}</li>
                ))}
              </ul>
            ) : (
        ...
        <CardContent>
          ...
          <ScrollArea className="h-[200px] mt-4">
             {unmarkedAttendees.length > 0 ? (
              ...
              <p className="text-sm text-muted-foreground italic">Todos los asistentes han sido marcados.</p>
            )}
          </ScrollArea>
        </CardContent>
      </Card>
