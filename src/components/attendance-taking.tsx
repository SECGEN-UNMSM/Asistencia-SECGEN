    ...
    <div className="container mx-auto p-4 space-y-6">
      <div className="flex flex-col lg:flex-row gap-6">
        <Card className="shadow-xl lg:flex-grow">
          <CardHeader>
            <CardTitle className="text-3xl font-bold text-center text-primary">Tomar Asistencia</CardTitle>
            <Progress value={progress} className="w-full mt-2" aria-label={`${progress.toFixed(0)}% completado`} />
            <p className="text-center text-muted-foreground mt-1">{`Asistente ${currentIndex + 1} de ${attendees.length}`}</p>
          </CardHeader>
          {currentAttendee && (
            <CardContent className="space-y-6">
              <div className="text-center p-6 bg-secondary/50 rounded-lg shadow">
                <h2 className="text-4xl font-semibold text-primary my-2" data-ai-hint="person name">{currentAttendee.name}</h2>
              </div>
              ...
              <div className="flex justify-center items-center space-x-4">
                ...
                <Button onClick={handleNext} disabled={currentIndex === attendees.length - 1} variant="outline" size="lg">
              ...
              <RadioGroup
                ...
                onValueChange={(status) => updateAttendeeStatus(currentAttendee.id, status as 'present' | 'absent' | 'unmarked')}
          ...
          <CardFooter className="flex justify-center">
            <Button onClick={() => router.push('/summary')} size="lg" className="bg-accent hover:bg-accent/90 text-accent-foreground">
        ...
        <Card className="shadow-xl lg:w-full lg:max-w-xs xl:max-w-sm">
            <CardHeader>
                <CardTitle className="text-2xl text-center text-primary">Resumen</CardTitle>
            </CardHeader>
            ...
            {filteredAttendees.length > 0 ? (
              filteredAttendees.map((attendee, index) => (
                <React.Fragment key={attendee.id}>
                  <div 
                    className={`flex justify-between items-center p-3 cursor-pointer hover:bg-secondary/70 ${attendee.id === currentAttendee?.id ? 'bg-secondary shadow-md ring-2 ring-primary' : ''}`}
                    onClick={() => handleAttendeeSelect(attendee.id)}
                    role="button"
                    tabIndex={0}
                    onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { handleAttendeeSelect(attendee.id); } }}
                    aria-label={`Seleccionar ${attendee.name}`}
                  >
                    <span className={`font-medium ${attendee.id === currentAttendee?.id ? 'text-primary' : ''}`}>{attendee.name}</span>
                    <span className={`text-sm capitalize font-semibold ${
                        attendee.status === 'present' ? 'text-green-600' :
                        attendee.status === 'absent' ? 'text-red-600' :
                        '' // No text for unmarked
                    }`}>
                      {attendee.status === 'present' ? 'Asistió' :
                       attendee.status === 'absent' ? 'No Asistió' :
                       ''}
                    </span>
                  </div>
                  {index < filteredAttendees.length - 1 && <Separator className="my-0" />}
                </React.Fragment>
              ))
            ) : (
              <p className="p-4 text-center text-muted-foreground">No se encontraron asistentes con ese nombre.</p>
            )}
          </ScrollArea>
        </CardContent>
      </Card>
