  ...
  const handleFileUpload = async () => {
    ...
    reader.onload = (e) => {
      try {
        ...
        let names: string[];
        if (lines.length > 0 && /name|nombre|attendee|asistente|participant|participante/i.test(lines[0].split(',')[0])) {
        ...
        if (names.length === 0) {
          toast({
            ...
            description: "El archivo CSV no contiene nombres válidos en la primera columna.",
            variant: "destructive",
          });
          setIsLoading(false);
          return;
        ...
        toast({
          ...
          description: "No se pudo procesar el archivo CSV. Asegúrate de que sea un CSV válido.",
          variant: "destructive",
        });
      } finally {
        setIsLoading(false);
  ...
  return (
    <Card className="w-full max-w-md shadow-lg">
      <CardHeader>
        ...
        <CardDescription>Selecciona un archivo CSV que contenga la lista de nombres de los asistentes.</CardDescription>
      </CardHeader>
